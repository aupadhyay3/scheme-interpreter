...
(define (nondecreaselist s)
    ; BEGIN PROBLEM 17
    (define (helper s ongoing)
        (cond ((null? s) (list (reverse ongoing)))
              ((or (null? ongoing) (>= (car s) (car ongoing))) 
                    (helper (cdr s) (cons (car s) ongoing)))
                (else
                    (cons (reverse ongoing) (helper s nil)))
        )
    )
(helper s nil)
)
