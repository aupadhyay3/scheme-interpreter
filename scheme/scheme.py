...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        evaluated_procedure = scheme_eval(first,env)
        validate_procedure(evaluated_procedure)
        list_args = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(evaluated_procedure, list_args, env)
        # END PROBLEM 4
...
def eval_all(expressions, env):
    ...
    return evaluated_val  # change this line
    # END PROBLEM 7
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_false_primitive(evaluated_expression):
            return False
        prev_val = evaluated_expression
        expressions = expressions.rest
    return prev_val
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    while expressions is not nil:
        evaluated_expression = scheme_eval(expressions.first, env)
        if is_true_primitive(evaluated_expression):
